...
const state = { manifestRequests:0, catalogRequests:0, metaRequests:0, streamRequests:0, mp4Requests:0, muxRequests:0, appleRequests:0, requests:[] };
function headers(){return {'Access-Control-Allow-Origin':'*','Access-Control-Allow-Methods':'GET, OPTIONS','Access-Control-Allow-Headers':'Origin, X-Requested-With, Content-Type, Accept','Cache-Control':'no-store','Content-Type':'application/json; charset=utf-8'};}
...
function makeStream(name,title,url,notWebReady,group,filename){const host=new URL(url).hostname.toLowerCase();if(!ALLOWED_HOSTS.has(host))throw new Error(`Unauthorized source host: ${host}`);return {name,title,url,behaviorHints:{notWebReady,bingeGroup:group,filename}};}
function manifest(){return {id:ADDON_ID,version:'0.4.20-probe',name:'Vietnam Sports Hub - Playback Matrix',description:'MP4 vs HLS playback compatibility probe.',resources:['catalog','meta','stream'],types:['tv'],idPrefixes:['sports:event:'],catalogs:[{type:'tv',id:'v0420-probe',name:'Vietnam Sports Playback Matrix'}],behaviorHints:{configurable:false}};}
...
function streamPayload(){
  const mp4=makeStream('MP4 control source','MP4 web-ready control source',MP4_URL,false,'v0420-mp4','BigBuckBunny_320x180.mp4');
  const mux=makeStream('Mux HLS comparison source','Mux HLS comparison source',MUX_URL,true,'v0420-mux','x36xhzz.m3u8');
  const apple=makeStream('Apple HLS comparison source','Apple HLS comparison source',APPLE_URL,true,'v0420-apple','bipbop_4x3_variant.m3u8');
