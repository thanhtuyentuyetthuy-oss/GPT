...
const MANIFEST = {
  ...
  version: '0.4.13-cloudflare',
  name: 'Vietnam Sports Hub',
  description: 'Vietnam sports addon deployed on Cloudflare Workers using cache-first test contracts.',
...
// This is a deployment fixture, not a new sports data source.
// The three IDs and FINISHED states mirror the V0.4.13 runtime test snapshot.
const CATALOG = [
  {
    id: 'sports:event:2397275',
    type: 'tv',
    name: '[DONE] Phoenix Rising vs Colorado Springs Switchbacks',
    description: 'State: FINISHED',
    genres: ['FOOTBALL'],
    behaviorHints: { defaultVideoId: 'sports:event:2397275' }
  },
  {
    id: 'sports:event:2397604',
    type: 'tv',
    name: '[DONE] Corpus Christi FC',
    description: 'State: FINISHED',
    genres: ['FOOTBALL'],
    behaviorHints: { defaultVideoId: 'sports:event:2397604' }
  },
  {
    id: 'sports:event:2407000',
    type: 'tv',
    name: '[DONE] Sporting',
    description: 'State: FINISHED',
    genres: ['FOOTBALL'],
    behaviorHints: { defaultVideoId: 'sports:event:2407000' }
  }
];
...
function metaFor(eventId) {
  ...
  return {
    meta: {
      id,
      type: 'tv',
      name: entry.name.replace(/^\[(LIVE|NEXT|DONE)\]\s*/, ''),
      description: 'Vietnam Sports Hub test deployment',
      videos: [{ id, title: entry.name.replace(/^\[(LIVE|NEXT|DONE)\]\s*/, '') }]
    }
  };
}
...
function streamFor(eventId) {
  if (eventId !== EVENT_ID) {
    return json({ error: `Test stream fixture is available only for event ${EVENT_ID}.` }, 404);
  }
  if (!isAuthorizedStream(TEST_STREAM_URL)) {
    return json({ error: 'Configured stream host is not allowlisted.' }, 503);
  }
  return json({
    streams: [{
...
export default {
  async fetch(request) {
    ...
    if (path === '/catalog/tv/vietnam-sports.json' || path === '/catalog/tv/vietnam-sports') {
      return json({ metas: CATALOG, cacheMaxAge: 60 });
    }
    ...
    if (streamMatch) {
      const eventId = streamMatch[1].replace(/^sports:event:/, '');
      return streamFor(eventId);
    }
